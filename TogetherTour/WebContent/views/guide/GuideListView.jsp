...
<div class=wrapper>
	...
	<div class=contents>
		...
		<select name=분류 class=selectList>
			<option>분류</option>
			<option>국내</option>
			<option>해외</option>
		</select>
		<select name=국가 class=selectList>
			<option>국가</option>
			<option>한국</option>
			<option>미국</option>
			<option>중국</option>
		...
		<select name=도시 class=selectList>
		    <option>도시</option>
			<option>제주도</option>
			<option>뉴욕</option>
			<option>상해</option>
		...
		<label>예상가격</label>
		<select name=최소가격 class=selectList>
			<option>최소가격</option>
			<option>0</option>
			<option>50000</option>
		...
		<select name=최대가격 class=selectList>
			<option>최대가격</option>
			<option>50000</option>
			<option>100000</option>
			<option>150000</option>
		...
		<div class='pagingArea' align='center'>
			<% if(!list.isEmpty()){ %>
			<!-- 맨 처음으로 -->
			<button onclick="location.href='<%= request.getContextPath() %>/list.guide?currentPage=1'">&lt;&lt;</button>
			...
			<!-- 맨 끝으로 -->
			<button onclick="location.href='<%= request.getContextPath() %>/list.guide?currentPage=<%= maxPage %>'">&gt;&gt;</button>
			<% } %>
